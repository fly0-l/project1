...
def train(X_train, y_train, n_estimators, projectname, num):
    ...
    for _ in range(n_estimators):
        ...
        model_filename_json = f'D:/pycharm/pythonProject2/加入数据采样/cnn_1/{projectname}-'+(str)(_)+'.json'
        with open(model_filename_json, 'w') as json_file:
            json_file.write(model_json)

        # 保存模型权重（H5格式）
        model_filename_h5 = f'D:/pycharm/pythonProject2/加入数据采样/cnn_1/{projectname}-'+(str)(_)+'.h5'
...
def load_models(projectname, n_estimators):
    ...
    for i in range(n_estimators):
        model = model_from_json(open(f'D:/pycharm/pythonProject2/加入数据采样/cnn_1/{projectname}-{i}.json').read())
        model.load_weights(f'D:/pycharm/pythonProject2/加入数据采样/cnn_1/{projectname}-{i}.h5', by_name=True)
        models.append(model)
    return models
