...
def get_train_test_dataset(df):
    ...
    # 对特征数据进行归一化处理
    scaler = MinMaxScaler()
    data_normalized = scaler.fit_transform(data)  # 归一化特征数据

    # 使用 ROS 进行过采样
    ros = RandomOverSampler(random_state=42)  # 设置随机种子
    data_resampled, labels_resampled = ros.fit_resample(data_normalized, labels)

    # 调整输入形状为 Conv1D 期望的三维输入： (batch_size, timesteps, features)
    data_resampled = np.expand_dims(data_resampled, axis=-1)

    return data_resampled, np.array(labels_resampled)
...
def train(X_train, y_train, n_estimators, lambda_ncl, projectname, num):
    ...
    for _ in range(n_estimators):
        ...
        model_filename_json = f'D:/pycharm/pythonProject2/加入数据采样/no_rf/{projectname}-'+(str)(_)+'.json'
        with open(model_filename_json, 'w') as json_file:
            json_file.write(model_json)

        # 保存模型权重（H5格式）
        model_filename_h5 = f'D:/pycharm/pythonProject2/加入数据采样/no_rf/{projectname}-'+(str)(_)+'.h5'
...
def load_models(projectname, n_estimators):
    ...
    for i in range(n_estimators):
        model = model_from_json(open(f'D:/pycharm/pythonProject2/加入数据采样/no_rf/{projectname}-{i}.json').read())
        model.load_weights(f'D:/pycharm/pythonProject2/加入数据采样/no_rf/{projectname}-{i}.h5', by_name=True)
        models.append(model)
    return models
